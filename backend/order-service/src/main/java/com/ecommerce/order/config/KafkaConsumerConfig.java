...
/**
 * Retry + Dead Letter Topic configuration for Kafka consumers. A message that keeps failing is
 * retried 3 times with a 1s pause, then published to "&lt;topic&gt;.DLT" instead of blocking the
 * partition forever (no infinite retry loop) or being silently dropped.
 */
@Configuration
